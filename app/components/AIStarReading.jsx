...
//     if (readings.length === 0 && !advice && !numerologyIntro) {
//         return <p className="text-gray-300 animate-pulse">Generating your reading...</p>;
//     }

//     return (
//         <div className="mt-4 space-y-4 max-w-2xl">
//             {numerologyIntro && (
//                 <div className="p-4 bg-gradient-to-r from-purple-900 via-indigo-500 to-black rounded-lg text-white">
//                     <h3 className="text-2xl font-semibold gradient-title">Personal Influences:</h3>
//                     <p>{numerologyIntro}</p>
//                 </div>
//             )}
//             {readings.map((reading, index) => (
//                 <div key={index} className="p-4 bg-gradient-to-r from-purple-900 via-indigo-500 to-black rounded-lg text-white">
//                     <h3 className="text-2xl font-semibold gradient-title">{reading.header}</h3>
//                     <p className="text-sm text-purple-200">Theme: {reading.theme}</p>
//                     <p>{reading.content}</p>
//                 </div>
//             ))}
//             {Object.keys(adviceSections).length > 0 && (
//                 <div className="p-4 bg-purple-900/20 border border-purple-500 rounded-lg text-white space-y-4">
//                     <h3 className="font-semibold text-4xl gradient-title text-center">Overall Advice:</h3>
//                     {adviceSections.cardAcknowledgment && (
//                         <div>
//                             <h4 className="text-md font-semibold text-2xl text-yellow-300">Cosmic Selection</h4>
//                             <p>{adviceSections.cardAcknowledgment}</p>
...
export default function AIStarReading({
    ...
    useEffect(() => {
        ...
        const fetchReading = async () => {
            toastIdRef.current = toast.loading("Generating your cosmic reading...", {
                style: {
            ...
            try {
                ...
                adviceSections.cardAcknowledgment = cardAcknowledgmentSentences.join(" ");
                adviceSections.individualCard = cardParagraphs;
                adviceSections.lifeStage = lifeStageSentences.join(" ");
                adviceSections.zodiacGuidance = zodiacGuidanceSentences.join(" ");
                adviceSections.affirmations = affirmationsSentences.join(" ");

                console.log("Parsed Readings:", parsedReadings);
                console.log("Parsed Advice Sections:", adviceSections);
                console.log("Numerology Intro:", numerologyIntro);

                setReadings(parsedReadings);
                setAdviceSections(adviceSections);
                setNumerologyIntro(numerologyIntro);
                setError(null);
    ...
    return (
        <div className="mt-4 space-y-4 max-w-3xl">
            ...
            {readings.map((reading, index) => (
                <div key={index} className="p-4 bg-gradient-to-r from-purple-900 via-indigo-500 to-black rounded-lg text-white">
                    <h3 className="text-2xl font-semibold gradient-title">{reading.header}</h3>
                    <p className="text-sm text-purple-400">Theme: {reading.theme}</p>
                    <p>{reading.content}</p>
                </div>
            ))}
            {Object.keys(adviceSections).length > 0 && (
                <div className="p-4 bg-purple-900/20 border border-purple-600 rounded-lg text-white space-y-4">
                    <h3 className="font-semibold text-3xl text-center">Card Insights:</h3>
                    {adviceSections.cardAcknowledgment && (
                        <div>
                            <h4 className="text-lg font-semibold text-yellow-400">Cosmic Selection</h4>
                            <p>{adviceSections.cardAcknowledgment}</p>
                        </div>
                    )}
                    {adviceSections.individualCard && adviceSections.individualCard.length > 0 && (
                        <div>
                            <h4 className="text-lg font-semibold text-yellow-400">Individual Card Insights</h4>
                            {adviceSections.individualCard.map((cardParagraphs, index) => (
                                cardParagraphs && cardParagraphs.length > 0 && (
                                    <div key={index} className="mt-4">
                                        <h5 className="text-sm font-medium text-indigo-400">
                                            {cards[index]?.name} ({cards[index]?.position})
                                        </h5>
                                        {cardParagraphs.map((paragraph, paraIndex) => (
                                            <p key={paraIndex} className="text-white mt-2">{paragraph}</p>
                                        ))}
                                    </div>
                                )
                            ))}
                    ...
                    {adviceSections.lifeStage && (
                        <div>
                            <h4 className="text-lg font-semibold text-yellow-400">Life Stage Guidance</h4>
                            <p>{adviceSections.lifeStage}</p>
                        </div>
                    )}
                    {adviceSections.zodiacGuidance && (
                        <div>
                            <h4 className="text-lg font-semibold text-yellow-400">Zodiac-Specific Wisdom</h4>
                            <p>{adviceSections.zodiacGuidance}</p>
                        </div>
                    )}
                    {adviceSections.affirmations && (
                        <div>
                            <h4 className="text-lg font-semibold text-yellow-400">Empowering Actions and Affirmations</h4>
                            <p>{adviceSections.affirmations}</p>
                        </div>
                    )}
                </div>
